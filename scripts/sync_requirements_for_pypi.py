...
REQ_LLM_TXT = ROOT / "requirements-local-llm.txt"
...
# Read requirements-local-llm.txt
llm_reqs = []
if REQ_LLM_TXT.exists():
    with open(REQ_LLM_TXT, encoding="utf-8") as f:
        llm_reqs = [line.strip() for line in f if line.strip() and not line.startswith('#')]
...
if SETUP_CFG.exists():
    ...
    # Update llm-local extras_require
    if 'options.extras_require' not in config:
        config['options.extras_require'] = {}
    config['options.extras_require']['llm-local'] = '\n' + '\n'.join(f'    {r}' for r in llm_reqs)

    with open(SETUP_CFG, 'w', encoding='utf-8') as f:
        config.write(f)
    print(f"Updated install_requires and llm-local extras in {SETUP_CFG}")
...
if PYPROJECT.exists():
    ...
    # Update optional-dependencies llm-local
    if "optional-dependencies" not in pyproject_doc["project"]:
        pyproject_doc["project"]["optional-dependencies"] = {}
    pyproject_doc["project"]["optional-dependencies"]["llm-local"] = tomlkit.array(llm_reqs)

    with open(PYPROJECT, 'w', encoding='utf-8') as f:
        f.write(tomlkit.dumps(pyproject_doc))
    print(f"Updated dependencies and llm-local extras in {PYPROJECT}")
